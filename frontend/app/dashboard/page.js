...
const TYPE_META = {
  hiring_spike:      { color:"#7C3AED", label:"Hiring Spike",    icon:"📈" },
  new_country_page:  { color:"#00F0FF", label:"Expansion",       icon:"🌍" },
  pricing_change:    { color:"#f59e0b", label:"Pricing Change",  icon:"💰" },
  leadership_change: { color:"#a855f7", label:"Leadership",      icon:"👤" },
  new_product:       { color:"#22c55e", label:"New Product",     icon:"🚀" },
  compliance_update: { color:"#f97316", label:"Compliance",      icon:"⚖️"  },
  partner_page:      { color:"#6366f1", label:"Partner",         icon:"🤝" },
  expansion:         { color:"#00F0FF", label:"Expansion",       icon:"🌍" },
};
const STRENGTH_COLOR = { high:"#ef4444", medium:"#f59e0b", low:"#22c55e" };

function CRMStrip({ crmStats }) {
  if (!crmStats) return null;
  const items = [
    { label:"Contacts", value: crmStats.contacts || 0, sub: null, color:"#6366f1", href:"/dashboard/contacts", icon:"👥" },
    { label:"Accounts", value: crmStats.accounts || 0, sub: crmStats.arr ? `ARR ${crmStats.arr}` : null, color:"#00F0FF", href:"/dashboard/accounts", icon:"🏢" },
    { label:"Open Tasks", value: crmStats.open_tasks || 0, sub: crmStats.overdue_tasks > 0 ? `${crmStats.overdue_tasks} overdue` : "all on track", color: crmStats.overdue_tasks > 0 ? "#ef4444" : "#22c55e", href:"/dashboard/tasks", icon:"✅" },
    { label:"Pipeline", value: crmStats.pipeline || "₹0", sub: `${crmStats.open_deals || 0} open deals`, color:"#a855f7", href:"/dashboard/deals", icon:"💰" },
  ];
  return (
    <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:24 }}>
      {items.map(item => (
        <a key={item.label} href={item.href} style={{ textDecoration:"none" }}>
          <div style={{ background:"rgba(255,255,255,0.03)", border:`1px solid ${item.color}20`, borderRadius:10, padding:"14px 16px", transition:"border-color 0.15s, transform 0.15s", cursor:"pointer" }}
            onMouseEnter={e => { e.currentTarget.style.borderColor = item.color + "50"; e.currentTarget.style.transform = "translateY(-2px)"; }}
            onMouseLeave={e => { e.currentTarget.style.borderColor = item.color + "20"; e.currentTarget.style.transform = "translateY(0)"; }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8 }}>
              <span style={{ fontSize:11, color:"#737373", fontWeight:600, textTransform:"uppercase", letterSpacing:"0.05em" }}>{item.label}</span>
              <span style={{ fontSize:16 }}>{item.icon}</span>
            </div>
            <div style={{ fontSize:26, fontWeight:900, color:item.color, lineHeight:1, marginBottom:4 }}>{item.value}</div>
            {item.sub && <div style={{ fontSize:11, color: item.sub.includes("overdue") ? "#ef4444" : "#737373", fontWeight: item.sub.includes("overdue") ? 700 : 400 }}>{item.sub}</div>}
          </div>
        </a>
      ))}
    </div>
...
function NewSignalToast({ signal, onClose }) {
  const t = TYPE_META[signal?.signal_type] || { icon:"⚡", color:"#7C3AED" };
  return (
    <div style={{
...
function ActionPlanCard({ plan }) {
  const urgencyColor = { high:"#7C3AED", medium:"#f5a623", low:"#46d369" };
  if (!plan?.actions?.length) return null;
  return (
    <div style={{
      background:"linear-gradient(135deg, #130c0c 0%, #1a1111 100%)",
      border:"1px solid rgba(124,58,237,0.2)", borderRadius:10, padding:"24px 28px", marginBottom:28,
    }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:16 }}>
        <div>
          <div style={{ fontSize:11, fontWeight:700, color:"#7C3AED", letterSpacing:"0.12em", marginBottom:4 }}>
            📋 TODAY&apos;S ACTION PLAN
          </div>
        ...
        {plan.actions.map((a, i) => (
          <div key={i} style={{
            ...
            <a href={`/dashboard/signals?id=${a.signal_id}`} style={{
              padding:"6px 12px", borderRadius:6, background:"rgba(124,58,237,0.15)",
              color:"#7C3AED", fontSize:11, fontWeight:700, textDecoration:"none", flexShrink:0,
            }}>
              Act →
            </a>
          </div>
    ...
    <div style={{
      ...
      <div style={{ flex:1 }}>
        <div style={{ display:"flex", justifyContent:"space-between", marginBottom:6 }}>
          ...
          <span style={{ fontSize:12, fontWeight:700, color:"#7C3AED" }}>{rate}%</span>
        </div>
        <div style={{ height:6, background:"rgba(255,255,255,0.08)", borderRadius:3 }}>
          <div style={{ height:"100%", width:`${rate}%`, background:"linear-gradient(90deg,#00F0FF,#7C3AED)", borderRadius:3, transition:"width 1s ease" }}/>
        </div>
...
function SignalCard({ s, onAction, onDismiss, onDeal, justCreated, isNew }) {
  ...
  const avatarColor = ["#7C3AED","#00F0FF","#f59e0b","#22c55e","#a855f7","#6366f1"][
    (s.company_name?.charCodeAt(0) || 0) % 6
  ];
  return (
    <div style={{
      background: isNew ? "rgba(124,58,237,0.04)" : "#1a1a1a",
      borderRadius:8, padding:"20px 24px", marginBottom:8,
      border: isNew ? "1px solid rgba(124,58,237,0.3)" : "1px solid rgba(255,255,255,0.06)",
      ...
      onMouseLeave={e => { e.currentTarget.style.background= isNew ? "rgba(124,58,237,0.04)" : "#1a1a1a"; e.currentTarget.style.transform="translateX(0)"; }}
    >
        ...
        <div style={{ display:"flex", alignItems:"flex-start", gap:12, flex:1 }}>
          ...
          <div style={{ flex:1 }}>
            <div style={{ display:"flex", gap:6, flexWrap:"wrap", marginBottom:6 }}>
              {isNew && <span style={{ background:"#7C3AED22", color:"#7C3AED", padding:"2px 8px", borderRadius:20, fontSize:10, fontWeight:700 }}>NEW</span>}
      ...
      {!s.is_actioned ? (
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          <button onClick={() => onDeal(s)} style={{
            padding:"7px 16px", borderRadius:24, background:"#7C3AED", color:"#fff",
...
export default function DashboardPage() {
  ...
  // Load CRM overview stats
  useEffect(() => {
    Promise.all([
      apiFetch("/contacts?limit=1"),
      apiFetch("/accounts?limit=1"),
      apiFetch("/tasks?status=open"),
      apiFetch("/forecast?period=month"),
    ]).then(([c, a, t, f]) => {
      const fmtArr = v => v >= 1e7 ? `₹${(v/1e7).toFixed(1)}Cr` : v >= 1e5 ? `₹${(v/1e5).toFixed(1)}L` : `₹${Math.round(v||0).toLocaleString("en-IN")}`;
      setCrmStats({
        contacts: c.total || 0,
        accounts: a.total || 0,
        arr: a.total_arr ? fmtArr(a.total_arr) : null,
        open_tasks: t.stats?.open || 0,
        overdue_tasks: t.stats?.overdue || 0,
        pipeline: fmtArr(f.summary?.pipeline_value || 0),
        open_deals: f.summary?.open_deals || 0,
      });
    });
  }, []);
    ...
    <div>
      ...
      {toast && (
        <div style={{
          ...
          border: `1px solid ${toast.type === "success" ? "#46d369" : toast.type === "info" ? "#0071eb" : "#7C3AED"}`,
      ...
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24, flexWrap:"wrap", gap:12 }}>
        ...
        <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
          ...
          <button onClick={() => seed()} disabled={seeding} style={{
            ...
            background: seeding ? "rgba(124,58,237,0.4)" : "#7C3AED",
      ...
      {/* CRM Overview */}
      <CRMStrip crmStats={crmStats} />

      {/* Stats row */}
      <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:10, marginBottom:20 }}>
        <StatCard label="Total Signals"  value={stats.total || 0}          color="#fff"     />
        <StatCard label="High Priority"  value={stats.high_priority || 0}  color="#ef4444"  sub={stats.high_priority > 0 ? "Act today" : null} />
        <StatCard label="Actioned"       value={stats.actioned || 0}        color="#22c55e"  sub={plan?.streak > 0 ? `🔥 ${plan.streak}-day streak` : null} />
        <StatCard label="In Pipeline"    value={plan?.stats?.active_deals || 0} color="#f59e0b" />
      </div>
      ...
      {!loading && feed.length === 0 && (
        <div style={{ background:"#1a1a1a", borderRadius:10, padding:"72px 32px", textAlign:"center", border:"1px solid rgba(255,255,255,0.06)" }}>
          ...
          <button onClick={seed} disabled={seeding} style={{
            padding:"14px 36px", borderRadius:24, background:"#7C3AED", color:"#fff",
        ...
        <>
          ...
          {topSig && (
            <div style={{ marginBottom:28 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#7C3AED", letterSpacing:"0.12em", marginBottom:10 }}>⚡ SIGNAL OF THE DAY</div>
              <div style={{
                background:"linear-gradient(135deg, #1a1a1a 0%, #1f1a1a 100%)",
                border:"1px solid rgba(124,58,237,0.25)", borderRadius:10, padding:"28px 32px",
                position:"relative", overflow:"hidden",
              }}>
                <div style={{ position:"absolute", top:0, right:0, width:240, height:240, background:"radial-gradient(circle, rgba(124,58,237,0.07) 0%, transparent 70%)", pointerEvents:"none" }}/>
                {(() => {
                  ...
                  return (
                    <>
                      <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginBottom:12 }}>
                        ...
                        <span style={{ background:"rgba(124,58,237,0.15)", color:"#7C3AED", padding:"3px 10px", borderRadius:20, fontSize:11, fontWeight:700 }}>Score {topSig.score}/10</span>
                      ...
                      {!topSig.is_actioned && (
                        <div style={{ display:"flex", gap:8 }}>
                          <button onClick={() => createDeal(topSig)} style={{ padding:"10px 22px", borderRadius:24, background:"#7C3AED", color:"#fff", fontWeight:700, fontSize:13, cursor:"pointer", border:"none" }}>+ Add to Pipeline</button>
